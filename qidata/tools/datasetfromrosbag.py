...
# Local modules
from qidata import QiDataSet, DataType, qidatafile
...
if _rosbag_conversion_enabled:
	...
	def makeQiDataSetFromROSbag(rosbag_path, topics_to_save):
		"""
		...
		with RosBag(rosbag_path, 'r') as bag:
			...
			for (topic_name, msg, timestamp) in bag.read_messages(
				...
				if topics_info[topic_name][0] == "sensor_msgs/Image":
					...
					with qidatafile.open(os.path.join(
					       output_folder,
					       filename
					    ),"w") as _f:
	...
	def makeQiDataSetFromROSbag(rosbag_path, topic_to_prefix):
		raise Exception(
